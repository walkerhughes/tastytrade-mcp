"""Tier-2 eval runner: drive the tools with an LLM (or an oracle script) and score.

Two modes:
  --dry-run   Execute each case's oracle `script` directly against the mock API and check
              `script_check`. No API key needed; validates wiring + verifier logic (CI-safe).
  (default)   Drive Claude via the Anthropic API in a tool-use loop against the same tools,
              then check the final answer. Records tool calls, tokens, and latency.

Both modes install a mock-API-backed client so runs are deterministic and free of real
credentials or live market data.

Usage:
  uv run python -m evals.run --dry-run
  ANTHROPIC_API_KEY=... uv run python -m evals.run --model claude-fable-5
"""
...
CASES_DIR = os.path.join(os.path.dirname(__file__), "tier2", "cases")
...
async def run_oracle(case: dict) -> dict:
    """Execute the case's deterministic script and check it — the verifier sanity check."""
    install_mock_client()
    last: Any = None
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    print()
    for r in results:
        line = f"{r['name']:28} {'✓' if r['passed'] else '✗':5}"
        if not args.dry_run:
