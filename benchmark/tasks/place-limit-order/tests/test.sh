#!/usr/bin/env bash
# Verify the agent actually transmitted a market order to buy 5 AAPL, by inspecting the
# orders the mock API recorded to MOCK_STATE_FILE.
set -euo pipefail
APP_DIR="${APP_DIR:-/app}"
...
with open(sys.argv[1]) as fh:
    for line in fh:
        ...
        for leg in order.get("legs", []):
            ...
            if sym == "AAPL" and qty == 5 and "buy" in action:
                ok = True
